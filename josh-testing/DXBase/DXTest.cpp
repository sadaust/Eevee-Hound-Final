...
void DXTest::init(HWND& hWnd, HINSTANCE& hInst,bool bWindowed) {
	...
	testBullVec.ActivateABullet(D3DXVECTOR3(10,4,2),D3DXVECTOR3(-2,0,0),testCube4.primInfo);
	testBullVec.ActivateABullet(D3DXVECTOR3(37,4,3),D3DXVECTOR3(-7,0,0),testCube4.primInfo);
	testBullVec.ActivateABullet(D3DXVECTOR3(19,5,2),D3DXVECTOR3(-3,-.2,0),testCube4.primInfo);
	testBullVec.ActivateABullet(D3DXVECTOR3(0,5,0),D3DXVECTOR3(1,-1,1),testCube4.primInfo);
	...
	for(int i = 0; i<numCon;++i) {
		...
		if(i == 0) {
			...
			if(state.Gamepad.bLeftTrigger > 0) { 
				testBullVec.ActivateABullet(temp.cam_pos,D3DXVECTOR3(1,0,0),testCube4.primInfo);
			}
		}
